...
function getRemoteBobUrl() {
    ...
    try {
        ...
        throw createBobError(
            "BOB_REMOTE_URL must be a valid HTTP(S) URL.",
            503
        );
    }
}
...
function parseBobResult(stdout) {
    ...
    try {
        ...
        throw createBobError(
            "IBM Bob returned an invalid JSON response.",
            502
        );
    }
    ...
    if (
        ...
        typeof result.last_message !== "string"
    ) {
        throw createBobError(
            "IBM Bob did not complete the repository analysis successfully.",
            502
        );
    }
...
function parseBobStreamResult(stdout) {
    ...
    for (const line of String(stdout || "").split(/\r?\n/)) {
        ...
        try {
            ...
            throw createBobError(
                "IBM Bob returned an invalid stream JSON response.",
                502
            );
        }
    }
...
function createBobExecutionError(
    ...
    return createBobError(
        "IBM Bob could not complete the repository analysis.",
        502
    );
}
...
function redactDiagnosticText(value) {
    ...
    if (process.env.BOB_API_KEY) {
        diagnosticText = diagnosticText.replaceAll(
...
function describeBobArgs(args, workspace) {
    return args.map((argument, index) => {
        ...
        if (args[0] === "run" && index === args.length - 1) {
            return "<prompt>";
        }
...
function runBobProcess({
    ...
    return new Promise((resolve, reject) => {
        ...
        const diagnostics = createProcessDiagnostics({
            executable,
            args,
            operation,
            timeoutMs,
        ...
        timeoutTimer = setTimeout(() => {
            console.warn(
                "IBM Bob process timeout reached",
                {
                    ...
                    workspaceId,
                    stdoutReceived:
                        diagnostics.stdoutReceived,
                    stderrReceived:
                        diagnostics.stderrReceived,
                    stdoutBytes:
        ...
        const finishProcess = () => {
            ...
            const outputPreview =
                allowOutputPreview
                    ? {
                          stdout: redactDiagnosticText(
                              Buffer.concat(
                                  stdout
                              ).toString("utf8")
                          ),
                          stderr: redactDiagnosticText(
                              Buffer.concat(
                                  stderr
                              ).toString("utf8")
                          ),
...
async function requestRemoteBob(
    remoteUrl,
    endpoint,
    options = {}
) {
    ...
    try {
        ...
        if (!response.ok) {
            console.error(
                "Remote Bob service returned an unsuccessful response",
                {
                    operation,
            ...
            throw createBobError(
                "Remote Bob service could not complete the request.",
                502
            );
        }
        ...
        try {
            ...
            throw createBobError(
                "Remote Bob service returned invalid JSON.",
                502
            );
        }
    } catch (error) {
        ...
        if (error?.name === "AbortError") {
            throw createBobError(
                `Remote Bob ${operation} timed out.`,
                504
            );
        }

        console.error(
            "Remote Bob service request failed",
            {
                operation,
        ...
        throw createBobError(
            "Remote Bob service is unavailable.",
            503
        );
    } finally {
...
async function runRemoteBobHealthCheck(
    remoteUrl = getRemoteBobUrl()
) {
    if (!remoteUrl) {
        throw createBobError(
            "Remote Bob service is not configured.",
            503
        );
    }

    console.log(
        "Using remote Bob service",
        {
    ...
    if (health?.ok !== true) {
        throw createBobError(
            "Remote Bob service health check did not succeed.",
            502
        );
    }

    console.log(
        "Remote Bob health check successful",
        {
...
async function runBobHealthCheck() {
    ...
    if (remoteUrl) {
        return runRemoteBobHealthCheck(
            remoteUrl
        );
    }
...
async function runRemoteBob(
    ...
    console.log(
        "Sending repository analysis to remote Bob service",
        {
            workspaceId,
            host:
                new URL(
    ...
    if (
        ...
        throw createBobError(
            "Remote Bob service returned an invalid analysis response.",
            502
        );
    }
    ...
    console.log(
        "Remote Bob repository analysis successful",
        {
            workspaceId,
            taskId:
                bobResult.stats
...
async function generateDocumentation(
    ...
    /*
     * IMPORTANT:
     * When Render uses remote Bob, the Render filesystem
     * is NOT available to the local Bob process.
     *
     * Therefore send repositoryContext directly to the
     * remote Bob service instead of asking remote Bob to
     * read Render's workspace.
     */
    if (getRemoteBobUrl()) {
        return runRemoteBob(
    ...
    /*
     * Local Bob mode.
     * In this mode Bob can directly access the workspace.
     */
