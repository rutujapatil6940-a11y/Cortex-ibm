...
void mainImage(out vec4 o, vec2 C) {
  ...
  float degree =
    noise(vec2(t*0.1,tuv.x*tuv.y)*uNoiseScale);

  tuv.y *= 1.0/ratio;

  tuv *= Rot(
    radians((degree-0.5)*uRotationAmount+180.0)
  );
  ...
  tuv.x +=
    sin(tuv.y*frequency+warpTime)/amplitude;

  tuv.y +=
    sin(tuv.x*(frequency*1.5)+warpTime)
    /(amplitude*0.5);
  ...
  vec3 layer1 =
    mix(colDark,colOrg,S(edge0,edge1,blendX));

  vec3 layer2 =
    mix(colOrg,colLav,S(edge0,edge1,blendX));

  vec3 col =
    mix(layer1,layer2,S(v0,v1,tuv.y));
  ...
  float grain =
    fract(
      sin(dot(grainUv,vec2(12.9898,78.233)))
      *43758.5453
    );
